//C�digo desenvolvido por Igor Anthony Lara e Jo�o Paulo Andrade Paiva.
...
int main()
{
    ...
    double peso_aux = 0;
    scanf("%d", &N);
    while(N > 100 || N < 1){//la�o apenas para garantir que o usuario nunca digite um N alem do suportado
        printf("numero de vertices alem do suportado! digite um valor novamente at� 100 vertices:");
        scanf("%d", &N);
    }
    ...
    {
        scanf("%s", aux);
        if((strcmp(aux,"+")==0))//aqui come�a uma sequencia de if/else if pra saber qual comando o usuario deseja colocar
        {
            scanf("%d %d %d", &A, &B, &W);
        ...
        {
            scanf("%d %d", &A, &B);
            if(lista->listaAdj[A]!=NULL && lista->listaAdj[B]!=NULL){//caso exista vizinhos ele vai executar a bfs, se n�o ele nem vai executar e j� vai imprimir que n�o existe caminho
                BFS(lista, A, B);
            }else{
                    printf("(sem caminho)\n");
            }
...
{
    nodo *aux;
    nodo *novoA = criaNodo(B, W);//� criado um novo nodo de peso W
    ...
    if(cont == 1)return;
    insere_aresta(lista, B, A, W, cont + 1);//fun��o recursiva para colocar no B agora, ent inverte os valores na chamada da fun��o e como precisa que execute mais uma vez apenas, foi feito um contador para isso
}
double imprime_peso(ListaDeVertice *lista, int A, int B)//fun��o para imprimir peso
{
    double i = 0;
    nodo *aux;
    ...
    if(i == 0)//se o i for = 0, quer dizer que n�o existe essa aresta
    {
        i = DBL_MAX;//ai � passado o DBL_MAX para representar o infinito(que n�o existe peso) e assim o computador entende
    }
    else
    {
...
void imprime_matriz(ListaDeVertice *lista)//imprime a matriz de adj
{
    int i, j;
    nodo *aux;
    double **matriz_adj = (double**)malloc((sizeof(double*))*(lista->tam+1));//aloco a quantidade linhas, ela � criada como double por conta do uso do DBL_MAX para representar infinito
    ...
    for(i = 1; i <= lista->tam; i++)//percorro linha
    {
        ...
        {
            if(matriz_adj[i][j] == DBL_MAX)//se for DBL_MAX, � pq n�o existe aresta, ent imprime "-"
            {
                printf("- ");
            }
            else
...
void vizinhos(ListaDeVertice *lista, int A)//imprime todos os vizinhos
{
    ...
    if(aux == NULL)//se a lista for null, � pq n�o existe nenhum vizinho
    {
        printf("(nenhum)");
    }
    else
...
void popular(ListaDeVertice *lista)//imprime o vertice com mais vizinhos
{
    ...
    nodo *aux;
    int vetor_popularidade[lista->tam + 1];//declaro um vetor para salvar a quantidade de vizinhos, cada posi��o do vetor � um vertice
    for(i = 1; i <= lista->tam; i++)
    {
    ...
    for(i=1; i <= lista->tam; i++)
    {
        for(aux = lista->listaAdj[i]; aux != NULL; aux = aux->prox)//la�o para percorrer os vizinhos dos vertices e somar em sua posi��o + 1 caso exista vizinho
        {
    ...
    for(i = 2; i < lista->tam; i++)//for usado para percorrer o vetor popularidade
    {
        ...
        {
            if(maior == vetor_popularidade[i]){//condi��o caso sejam iguais
                if(maior_i > i){
                    maior_i = i;
                }
            }else{
                maior = vetor_popularidade[i];//variavel maior recebe o conteudo
                maior_i = i;//recebe qual � o vertice de maior valor
            }
...
double DFS(ListaDeVertice *lista, int vertice, double peso_total)//Essa fun��o percorre a lista de adj e retorna o seu peso
{
    nodo *aux = lista->listaAdj[vertice];//recebo o endere�o da lista desse vertice no aux
    lista->visitado[vertice] = 1;//marca como visitado
    printf("%d", vertice);//imprime o vertice
    for(;aux!=NULL; aux=aux->prox)//la�o para percorrer os vizinhos
    {
        peso_total+=aux->peso;//soma ao peso j� existente com o peso da aresta
        if(lista->visitado[aux->info]==0)
        {
            printf(" ");
            peso_total = DFS(lista, aux->info, peso_total);//chamada recursiva para verificar se os seus vizinhos j� foram visitados, se n�o foram, vai fazer a msm chamada recursiva at� todos serem visitados
        }
    }
    return peso_total;
}
void DFS2(ListaDeVertice *lista, int i)//Essa fun��o dfs2 vai percorrer os vertices
{
    int componentes = 0;
    for(int j = i; j <= lista->tam; j++)
    {
        if(lista->visitado[j] == 0)//se ele n�o foi visitado, vai visitar e imprimir todos os seus vizinhos e seu peso
        {
            printf("[");
            printf("], peso total %.0f\n", (DFS(lista, j, 0)/2));//como ele percorre o caminho de ida e volta, ent�o o retorno � dividido por 2
            lista->visitado[j] = 1;
            componentes++;
        }
    }
...
void BFS(ListaDeVertice *lista, int A, int B)
{
    int enfilerado[(lista->tam)+1];//Esse vetor foi criada para me dizer se o vertice foi enfilerado, ele foi criado com o tam do vertice + 1, pois n�o existe vertice 0.
    int pai[(lista->tam)+1];//vetor que vai ser usado para saber o menor caminho
    int nivel[(lista->tam)+1];//me mostra a quantidade de arestas at� ele
    for(int i = 1; i <= lista->tam; i++)
    {
        nivel[i] = 0;
        enfilerado[i] = 0;
    ...
    nodo *aux;
    push(f, A);//enfilero o primeiro valor que � o do vertice A
    enfilerado[A] = 1;
    while(vazia(f)==0)//enquanto essa fun��o n�o me retornar 1, quer dizer que ela n�o est� zerado
    {
        x = pop(f);//
        for(aux = lista->listaAdj[x]; aux!=NULL; aux = aux->prox)//for usado para percorrer a lista de adj e enfilerar todos os vizinhos
        {
            if(enfilerado[aux->info] == 0)//condi��o que vai verifica se o viznho foi enfilerado ou n�o, se n�o for ele vai entrar
            {
                enfilerado[aux->info] = 1;//marca como enfilerado
                push(f,aux->info);//enfilera
                pai[aux->info] = x;//salva no pai quem � o pai desse vertice
                nivel[aux->info] = nivel[x] + 1;//o nivel dele � o nivel do pai + 1
            }
        }
    }
...
void *tira_aresta(ListaDeVertice *lista, int A, int B, int parada)/*essa fun��o � usada para remover uma aresta da lista de adj*/
{
    ...
    while(aux!=NULL)
    {
        ...
        {
            if(cont >= 1)//quer dizer que existem valores ap�s esse vertice e o valor desejado nao � o primeiro valor na ordem da lista
            {
                anterior->prox = aux->prox;
            }
            if(cont == 0 && aux->prox != NULL)//ele est� como primeiro na ordem da lista de adj e existem valores ap�s ele
            {
                lista->listaAdj[A] = aux->prox;
            }
            else if(cont == 0)//nao existem valores ap�s esse vertice
            {
    ...
    if(parada == 1) return;//condi��o de parada
    tira_aresta(lista, B, A, parada + 1);//chamada recursiva agr para o remover no outro vertice a aresta
}
double peso(ListaDeVertice *lista, int invertido[], int total_vertices, int i, double peso_t)//fun��o usada para retornar o valor do peso do caminho o pai � chamado de invertido, ele � usado para acessar os
{                                                                                            //valores do caminho, assim essa fun��o consegue percorrer o menor caminho e armazenar o peso
...
void imprime_pai(int p[], int A, int i, int aux_inverte[], int vertice, int vertices_total)//fun��o para imprimir o menor o caminho, esse caminho est� no vetor pai
{                                                                                          //mas como ele est� invertido, foi criado um vetor para receber os valores de maneira recursiva, ai ele come�o da ultima casa
                                                                                           //assim o caminho inverso � colocado na msm ordem, logo ap�s todos valores serem passados ele imprime esse vetor
    int cont = 0;
    if(i == A)
    {
        int z=0;
...
void destroy(nodo *limpeza)//Fun��o recursiva usada para percorrer a lista de adj e liberar os nodos criados
{
...
void destroy2(ListaDeVertice *lista){//fun��o principal para liberar da memoria, � usada um la�o for para analisar se existem nodos na lista de adj
    for(int i = 1; i <= lista->tam; i++){
